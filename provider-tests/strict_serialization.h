#ifndef ED301V1_STRICT_SERIALIZATION_H
#define ED301V1_STRICT_SERIALIZATION_H
...
#define ED301V1_PKCS8_DER_BYTES \
    (sizeof(ED301V1_PKCS8_PREFIX) + ED301V1_SEED_BYTES)
#define ED301V1_SPKI_DER_BYTES \
    (sizeof(ED301V1_SPKI_PREFIX) + ED301V1_PUB_BYTES)

_Static_assert(ED301V1_PKCS8_DER_BYTES == 62,
    "v1 PKCS#8 must be exactly 62 bytes");
_Static_assert(ED301V1_SPKI_DER_BYTES == 58,
    "v1 SPKI must be exactly 58 bytes");

static inline EVP_PKEY *ed301v1_strict_der_import(
    OSSL_LIB_CTX *libctx,
    ...
    int is_public)
{
    const unsigned char *prefix = is_public
        ? ED301V1_SPKI_PREFIX : ED301V1_PKCS8_PREFIX;
    const size_t prefix_length = is_public
        ? sizeof(ED301V1_SPKI_PREFIX) : sizeof(ED301V1_PKCS8_PREFIX);
    const size_t expected_length = prefix_length + ED301V1_SEED_BYTES;

    if (data == NULL || data_length != expected_length
            || CRYPTO_memcmp(data, prefix, prefix_length) != 0)
        return NULL;
    if (is_public)
        return ed301v1_key_from_public(
            libctx, data + prefix_length, ED301V1_PUB_BYTES);
    return ed301v1_key_from_seed(libctx, data + prefix_length);
}

static inline EVP_PKEY *ed301v1_strict_pem_import(
    OSSL_LIB_CTX *libctx,
    const unsigned char *data,
    size_t data_length,
    int is_public)
