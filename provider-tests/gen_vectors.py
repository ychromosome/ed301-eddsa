#!/usr/bin/env python3
"""Generate provider test fixtures from the repository's v1 vectors.

Reads only repository inputs:
  - inputs/v1/ed301-eddsa-v1.json
  - inputs/v1/ed301-eddsa-edge-v1.json
...
ORACLE = REPOSITORY / "inputs" / "v1"
POSITIVE = json.loads(
    (ORACLE / "ed301-eddsa-v1.json").read_text())
EDGE = json.loads(
    (ORACLE / "ed301-eddsa-edge-v1.json").read_text())
PARAM = json.loads(
    (REPOSITORY / "inputs" / "round4" / "upstream" / "ed301-v1"
     / "ed301-v1.json").read_text())
HIST = json.loads(
...
emit("#ifndef ED301V1_TEST_VECTORS_H")
emit("#define ED301V1_TEST_VECTORS_H")
...
from ed301_eddsa_v1 import reference  # noqa: E402  (bundled reference oracle)

POLICY_MESSAGE = b"v1 policy lane (test-only)"
...
def policy_challenge(commitment_encoding, public_encoding, message):
    return reference._hash_to_scalar((
        reference.domain(), commitment_encoding, public_encoding, message))[1]
...
emit(" * Ed301-EdDSA-v1 'empty' vector).  Used only to demonstrate that the")
emit(" * historical Ed301-Sig-v1 protocol does NOT verify as Ed301-EdDSA-v1. */")
...
emit("")
emit("#endif /* ED301V1_TEST_VECTORS_H */")
